...
static Type dispatchParse(AsmParser &parser, bool allowAny = true) {
  ...
  return StringSwitch<function_ref<Type()>>(key)
      ...
      .Case("float4e2m1x2", [&] { return LLVMFloat4E2M1x2Type::get(ctx); })
      .Case("float4e1m2x2", [&] { return LLVMFloat4E1M2x2Type::get(ctx); })
      .Case("hifloat4x2", [&] { return LLVMHiFloat4x2Type::get(ctx); })
      .Case("hifloat8", [&] { return LLVMHiFloat8Type::get(ctx); })
      .Case("float8e4m3", [&] { return LLVMFloat8E4M3Type::get(ctx); })
      .Case("float8e5m2", [&] { return LLVMFloat8E5M2Type::get(ctx); })
      .Case("float8e8m0", [&] { return LLVMFloat8E8M0Type::get(ctx); })
